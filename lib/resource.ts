...
export const getDistance = (A: Object3D, B: Object3D) => {
  return Math.sqrt(Math.pow(A.position.x - B.position.x, 2) + Math.pow(A.position.z - B.position.z, 2));
};

const spawnResource = (type: string, restrictedArea = [new Vector2(-50, 50), new Vector2(-50, 50)]): Resource => {
  ...
  const position = new Vector3(xMin + Math.random() * xRange, 0, zMin + Math.random() * zRange);
